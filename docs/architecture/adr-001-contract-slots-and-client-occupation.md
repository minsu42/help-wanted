...
> ## 구현 상태 — **D1~D4의 슬롯 기반은 P1-001로 반영됐다. D5 이후 직업 실현은 아직이다**
>
> 2026-08-10 P1-001 구현으로 `SlotName`·`SlotTruth`·`SlotProgress`,
> `Contract.slots`, `PlayerKnowledge.slotProgress`와 표시 파생 함수가 들어갔다.
> 7칸 어휘와 2값 `Limiter`의 최종 소유자는 ADR-004다.
>
> 다만 `Occupation`·`Client.occupation`과 직업별 `knows`/`tells` 실현은 P1-002 범위라
> 아직 코드에 없다. 따라서 **ADR 전체가 구현됐다고 인용하면 안 되고**, 슬롯 기반만
> 반영됐다고 구분해야 한다. 기존 의뢰는 과도기 `questKind='legacy'`와 빈 슬롯 맵을 써서
> P1-001 자체로는 RNG 소비가 늘지 않는다.
>
> D3 원문의 3값 `Limiter`는 변경 이력으로 남아 있고, 실제 타입은 ADR-004 D2의 2값이다.
...
2. **서버 없음** — GitHub Pages 정적 배포. NAN2026 예선용이며 ~~**영구 제약**이다~~
   → **2026-08-10 개정 (ADR-003).** **게임 상태를 서버에 두지 않는다는 것은 불변이다** —
   모든 상태는 클라이언트에 있고 Worker는 무상태다. 그러나 *"외부 API를 호출할 수 없다"*
   는 절반은 번복됐다: **전용 LLM 프록시 Worker 1개**가 열렸다 (`workers/intake-proxy/`).
   범용 중계가 아니라 이 게임 전용 엔드포인트이며, 열린 것은 그 하나뿐이다
3. **밸런스 하드코딩 금지** — 수치는 `balance.json`, 구조는 `quest-templates.json`
   *(2026-08-10: 원문에 있던 `questions.json`을 지웠다 — **그 파일은 한 번도 존재한 적이
   없고** 질문 카탈로그 개념 자체가 GDD 재작성에서 폐기됐다)*
4. **런타임 의존성 0개**
...
> ### ⚠ 2026-08-10 개정 — **어휘가 6개에서 7개가 된다.** 이 결정은 번복됐다
>
> 위 결정 원문은 남긴다. 바뀐 것은 의뢰서 확정 설계다: 의뢰서의 자동 기록이
> **7항목**(`의뢰 종류·대상·규모·장소·기한·경로·**특징 및 약점**`)으로 확정되면서
> **`'weakness'`(특징 및 약점)이 신설**된다. *"햇빛을 꺼림"* 같은 약점을 담는 칸이며,
> 대비 `C`를 가장 크게 올리는 항목이다.
>
> D1이 *"**닫힌 유니온이다**"* 라고 못박은 바로 그 유니온이 바뀌는 것이므로 —
> 즉 **타입 스키마 변경**이므로 — 로드맵의 「ADR을 언제 쓰는가」가 ADR을 요구한다.
> **여기서 타입을 고치지 않는다.**
>
> **소관은 ADR-004 D1**(`adr-004-intake-types-and-commission-schema.md`, Accepted).
> 그 ADR이 RNG 소비 순서에서 새 칸의 자리도 함께 정한다 (D6이 그 순서를
> *"저장 포맷의 일부처럼 취급한다"* 고 적었기 때문이다).
>
> **D1의 논증 자체는 반증되지 않았다** — *"의뢰 종류를 늘려도 유니온은 그대로다"* 는
> 여전히 참이고, 이번 변경은 종류가 늘어서가 아니라 **의뢰서라는 새 소비처가 생겨서**
> 일어났다. 어휘를 여는 것이 아니라 한 칸 더 닫는 것이다.
...
> ### ⚠ 2026-08-10 개정 — `'question'`은 이제 **어디서도** 나올 수 없다 (도달 불능 값)
>
> 위 결정 원문은 남긴다. 바뀐 것은 세계다: `intake-system.md` **v6.1**의 R4가
> **묻기를 무료로 만들고 질문 깊이 등급을 폐기**했다. 플레이어는 칸을 짚을 뿐이고
> 깊이 `L`은 칸의 다음 단계로 자동 결정되므로, **"뭉툭하게 물었다"는 사건이 발생하는
> 분기가 판정에 남아 있지 않다.** F2의 3분기는 `knows<L` / `tells<L` / 그 외뿐이다.
>
> 이 값을 지탱하던 근거도 함께 죽었다 — Alternatives의 *"`Limiter`를 2값 유지"* 기각
> 사유는 **질문 깊이 등급의 존재**였고, Verification의 `AC-F2-02` 행이 검증하려던
> 성질도 마찬가지다 (그 AC 번호는 v6.1 AC 목록에 더 이상 없다).
>
> **여기서 타입을 고치지 않는다.** `Limiter`에서 값을 빼는 것은 **타입 스키마 변경**이고,
> 로드맵의 「ADR을 언제 쓰는가」가 그것에 ADR을 요구한다. 이 ADR을 조용히 덮어쓰면
> *"결정이 언제 왜 바뀌었는지"* 가 사라진다.
> **소관은 ADR-004(청취 타입과 의뢰서 스키마)** — ADR-002 「다음 ADR 후보」 참조.
>
> *(2026-08-10 갱신)* **그 ADR이 쓰였다** —
> `adr-004-intake-types-and-commission-schema.md`, **Accepted**, **D2**가 이 값의
> 제거를 소유한다. 확정 설계에서 근거가 하나 더 굳었다: 의뢰서에 플레이어의 자유
> 입력이 **위험도 등급 하나뿐**이 되면서, *"벽이 플레이어 자신"* 인 상황이 청취
> 어디에도 남지 않았다.
...
- **ADR-004 — 청취 타입과 의뢰서 스키마.** **Accepted (2026-08-10)** —
  `adr-004-intake-types-and-commission-schema.md`. **이 ADR의 D1(칸 어휘 6개)과
  D3(`Limiter`의 `'question'`)을 명시적으로 번복하는 문서다.** 이 ADR 본문의 두 ⚠
  상자가 그리로 위임한 것이 그것이다. 원문은 여기 보존된다
- **ADR-002 — 지속성 포맷.** `ReadonlyMap` 직렬화, 스키마 버전 필드, 세이브 스캐밍 억제.
  재현 키는 ~~**매칭된 질문 id**~~ → *(2026-08-10 정정)* **`(칸, 행동, 재료?)` 시퀀스**이며,
  **로그 재생 방식은 기각**됐다 (GDD 「결정론」 엣지 케이스).
  > **이 한 줄이 두 번 낡았다** — 「질문 id」는 질문 카탈로그 모델의 것이라 GDD 3차
  > 개정에서 「분류 결과 `(노드 id, 화법)`」로 갔고, **v6.1이 분류를 통째로 폐기**하면서
  > 다시 순수 플레이어 입력으로 돌아왔다 (GDD R10·「결정론」). 소유자는 ADR-002다
- **부분 렌더링 경계 — 번호 미정.** ~~ADR-003~~ → **ADR-003 번호는 「런타임 LLM과 프록시」가
  가져갔다** (같은 날, 더 큰 결정이었다. 후보 예약은 번호를 소유하지 않는다).
  청취 화면의 텍스트 입력·부분 갱신 규약은 기존 6화면의 전체 재렌더 관용구를 **처음으로
  깨는 것**이므로 계층 경계 변경에 해당한다. 실제 구현량 실측 이후에 쓴다 —
  ADR-002 「다음 ADR 후보」가 이 항목의 현재 소유자다
