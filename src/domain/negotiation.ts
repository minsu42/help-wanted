 ...
 *
 * > 2026-08-09 개정 — 선불 축(`Offer.advanceRatio` / `NegotiationConfig.wAdvance`)을
 * > 제거했다. `NegotiationAxis`는 `'reward'` 단일 멤버 유니온으로 남겨둔다(P3의
 * > 「근거 기반 협상」이 축을 다시 늘릴 자리다). **잃은 것 하나**: 반박이 축을
 * > 지목하는 것이 `wealth`에 대한 정보 채널이었는데("선불은 도저히 안 되겠소") 축이
 * > 하나만 남아 그 채널이 사라졌다 — P3이 메울 자리다.
 *
 ...
 *
 * 지금은 `'reward'` 하나뿐이다. 타입 자체를 유니온으로 남겨둔 이유는 P3의 「근거
 * 기반 협상」이 축을 다시 늘릴 자리이기 때문이다 — 그때 여기에 멤버를 추가한다.
 */
export type NegotiationAxis = 'reward';
...
export interface NegotiationResult {
  ...
  /** 보상 축이 부담에 기여한 몫. 지금은 축이 하나뿐이라 {@link burden}과 같다 */
  readonly rewardBurden: number;
  /**
   * 의뢰인이 지목한 축. `'countered'`일 때만 존재한다.
   *
   * > 2026-08-09 개정 — 선불 축 제거로 `contestedAxisOf()`(기여도 비교 함수)가
   * > 사라졌다. `NegotiationAxis`가 `'reward'` 단일 멤버라 값은 항상 `'reward'`다.
   * > 원래 뜻(아래)은 축이 둘 이상일 때만 성립했다.
   * >
   * > 원문: **이 반박 자체가 숨은 상태에 대한 정보다** ("선불은 도저히 안 되겠소" →
   * > 이 사람은 자금이 없다). `urgency`와 `hasAlternative`를 소문으로 얻을 수 없게
   * > 만든 대신, 흥정 과정이 그 정보 채널이 된다.
   */
...
export function evaluateOffer(
  ...
  const measured = { burden, tolerance, rewardBurden };
  ...
  return {
    outcome: 'countered',
    ...measured,
    // 축이 'reward' 하나뿐이므로 반박은 항상 이것을 지목한다.
    contestedAxis: 'reward',
  };
}
...
//
// 이 축만 예외적이다 — 보상은 정보 없이도 부를 수 있지만(불리할 뿐), 위험 고지는
