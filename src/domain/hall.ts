/**
 * 길드 홀 출석 판정 — 이번 주 홀에 누가 있는지 정한다.
 *
 * 이 게임은 플레이어에게 "한 주 N회 대화권"을 주지 않는다. 대신 **그 주 홀에 누가
 ...
 *
 * ## 등급 상승이 "다음 주부터" 반영되는 이유는 이 파일에 없다
 *
 * 이 함수는 넘겨받은 `config.hallAttendanceMax`를 그대로 쓸 뿐이다. "다음 주부터"는
 * 호출자(`gameState.ts`의 `advanceWeek`)가 그 주 시점의 `currentTier`를 읽어 넘기는
 * 타이밍이 보장한다 — 등급을 올린 주의 `advanceWeek`는 이미 지나갔으므로, 다음
 * `advanceWeek` 호출 때 자연히 새 상한이 적용된다.
 *
 ...
 * 유일한 사람 저장소여야 한다. 여기 `Adventurer` 객체를 복제해 두면, 그 사람이 그 주
...
/**
 * 이번 주 길드 홀에 누가 있는지 뽑는다.
 *
 * 같은 `rng` 상태에서 부르면 같은 결과가 나온다 — **한 주에 한 번만** 불러야 한다.
 * 반환값을 어딘가에 저장해 두고 화면이 재렌더될 때마다 이 함수를 다시 부르면, 같은
 * 주인데 출석자가 바뀌는 모순이 생긴다. (저장 위치는 이 파일의 책임이 아니다 — 호출자
 * 쪽 문서 참조)
 *
