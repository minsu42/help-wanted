...
> **2026-08-10 최신 확정:** #12는 **8주 주간 진행(Weekly Cycle)**, #21은 저작된 선택형
> 대화와 보수 합의, #41은 길드 홀 게시판 복수 파견으로 구현한다. 매주 새 의뢰인 3명,
> 홀에서 주 마감 1회, 미배정 의뢰 이월이 현재 기준이다. 아래의 하루·15일·P3 대기 목록
> 전제와 충돌하면 이 문단과 `game-concept.md`가 우선한다.
...
| 12 | **주간 진행 (Weekly Cycle)** | Progression | **P1** | **Implemented** — 8주 캠페인. 매주 새 의뢰인 3명 순차 응대 → 길드 홀 복수 파견 → 홀에서 주 마감 1회 → 결과 큐 → 다음 주 | `game-concept.md`, `story-P1-012-day-flow.md` | 의뢰 생성, 청취(#21), 배정·게시(#41), 파견 판정(#10) |
...
| 21 | **청취 (Intake)** | Gameplay | **P1** | **Implemented** — 칸 선택 → 내 대사 → 표정 → 답 → 후속 선택. 자유 입력 없음. 무지/은폐, 인내 3, 책 재료, 선제 보수 제안과 수락·시세·위험 수당·역제시, 합의 뒤 위험도 날인 | **`design/gdd/intake-system.md`** | 의뢰 생성, 의뢰 템플릿(#24), 길드마스터북(#26), 의뢰인의 인내(#40) |
...
| 27 | ~~행동 횟수 (Action Budget)~~ | Progression | — | **폐기** — 질문 횟수와 주간 행동 횟수 제한을 두지 않는다. 압박 자원은 의뢰인별 인내(#40)뿐이다 | `intake-system.md` | — |
| 28 | **미배정 의뢰 이월 (Contract Carry-over)** | Gameplay | **P1** | **Implemented** — 날인했지만 파견하지 않은 의뢰는 길드 홀 게시판에 다음 주에도 유지된다 | `game-concept.md`, `assignment-dispatch.md` | 주간 진행(#12), 배정·게시(#41) |
...
| 39 | **8주 고정 캠페인 (Fixed 8-Week Campaign)** | Progression | **P1** | **Implemented** — 8주차 마감 결과를 모두 보여준 뒤 결산하며 종료 시 새 의뢰를 만들지 않는다 | `game-concept.md` | 주간 진행(#12), 결산(#17) |
...
| 41 | **배정·게시 (Assignment & Posting)** | Gameplay | **P1** | **Implemented** — 세 의뢰 응대 뒤 길드 홀 게시판에 합의·날인된 의뢰를 게시한다. 등급별 동시 파견 한도까지 복수 배정하며 파견 화면은 시간을 진행하지 않는다 | **`design/gdd/assignment-dispatch.md`** | 의뢰서 작성(#25), 모험가 명부(#5), 주간 진행(#12), 파견 판정(#10) |
