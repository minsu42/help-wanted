...
> ## 2026-08-10 — **이 조사가 근거를 댄 두 결정은 사라졌다. 그런데 §4.1의 경고는 오히려 더 정확해졌다.**
>
> 현행: `design/gdd/intake-system.md` v6.1.
>
> **무효 셋:**
>
> - **「R4 깊이 등급」** — v6.1의 R4는 깊이 등급이 아니다. 칸이 `미상 → 어렴풋 → 확실`
>   2단 사슬을 갖고, 응답은 `모른다 / 얼버무린다 / 말한다` 3분기다 (F2).
>   *(단, (나)를 고른 이유였던 **"뭉툭한 질문에도 부분 정보가 나온다"** 는 `어렴풋`
>   단계로 그대로 살아남았다 — §4.2의 L.A. Noire 처방은 유효하다)*
> - **「하루 행동 칸 12」** — **폐기.** 묻기·캐묻기·성공한 건네기·들이대기·넘어가기·
>   단정이 전부 무료다. 남은 자원은 **의뢰인의 인내(인당 3)** 이고 **헛발만** 깎는다
> - **§2의 *"서버 없음은 영구 제약이다"*** — **번복됐다.** ADR-003(2026-08-10)이 전용
>   Cloudflare Worker 프록시를 통한 런타임 LLM 호출을 허용한다. 게임 본체의 GitHub
>   Pages 배포는 불변이므로 §2의 나머지(심사자는 완주하지 않는다 · 첫 30분 · 1초/1MB
>   예산이 성능이 아니라 심사 리스크라는 지적)는 **전부 그대로 유효하다**
>
> ### §4.1은 죽지 않았다 — 이제 우리 이야기다
>
> > *"무비용 + 단일 등급 = 전부 눌러보기가 지배 전략."*
>
> 이 문장은 v6.1의 **묻기**를 정확히 묘사한다. 묻기는 무비용이고 칸은 단일 등급이다.
> 이 조사는 그 상태를 막으려고 하루 행동 칸을 세웠고, **v6.1은 그 방어선을 치웠다.**
> 대응 관계를 명시해 둔다 — 이것을 적어 두지 않으면 다음 사람은 방어선이 통째로
> 없어진 줄 안다:
>
> | Ace Attorney 2층 구조 | v6.1의 자리 |
> | ---- | ---- |
> | `Press` — 안전한 탐색, 무비용 | **묻기·캐묻기** (R4, 무료) |
> | `Present` — 확정, 5회 페널티 | **일깨우기·들이대기** (R5·R6) — 성공은 무료, **헛발만 인내 −1** |
>
> **「탐색은 싸고 확정은 비싸다」는 살아남았고, 「확정」의 자리만 옮겨 갔다.**
> 전부 눌러보기를 막는 것은 이제 셋이다: ① **선제 진술**이 순순히 줄 답을 미리 다
> 말해 잡일을 없앤다 (R3) ② **지식의 희소성** — 막힌 칸은 맞는 재료를 가진 사람만
> 연다 ③ **헛발 벌점** — 인내가 바닥나면 사람이 떠난다.
>
> **이 대응이 실제로 성립하는지가 프로토 검증 항목이다.** 성립하지 않으면 §4.1이
> 예언한 상태(*"전부 눌러라"*)로 그대로 떨어지며, 그때 되돌아올 곳이 이 절이다.
> `input-modality-2026-08-10.md` 부록의 Morrowind 사례가 같은 말을 하는 두 번째
> 독립 선례다.
>
> §1(장르 시장) · §3(YWBD 비교작 경고) · §4.2~§4.4 · §6은 손대지 않는다.
