...
> (개별 규약에 대한 개정 주석은 Implementation Deviations 절 안에도 있다.)
>
> **19개 스토리 중 개정 폭이 가장 큰 스토리다.**
>
> **폐기된 것**: **창구가 흥정 한 막이라는 전제 전체.** 개정된 창구는 세 막
> (청취 → 의뢰서 작성 → 흥정)이며, 이 스토리는 그중 마지막 막만 만들었다. 앞의 두 막이
> 통째로 없다 — 의뢰인을 캐묻는 입력 구조도, 받아쓰기 칸/판단 칸으로 나뉜 의뢰서도
> 존재하지 않는다. 컨셉이 *"창구가 비어 있었다"* 고 적은 것이 이 화면이다.
> 개별 항목: 제목의 "3축", AC-2의 선불 비율 조작, AC-5가 렌더하는 `'advance'` 반박
> 문장, 그리고 화면의 중심이 **대화창**이라는 배치 전제(개정 후에는 **작성 중인
> 서류**가 가장 크고 가장 촉각적이어야 한다).
> **대체·확장**: `production/roadmap.md` **P1**(청취 판정, 질문 카탈로그, 입력 3중
> 구조, 의뢰서 화면, 길드마스터북) 및 **P3**(근거 기반 협상).
> 새 요구사항은 그쪽이 소유한다. Implementation Deviations가 세운 **화면 모듈 규약
> 4개**(`mount*Screen` 핸들, 루트 단일 리스너 위임, 부분 갱신 조건, 바깥 세계 주입)와
> 한국어 조사 처리는 전부 생존하며 새 화면들도 이를 따른다.
...
3. 상태 변화는 전체 재렌더, **슬라이더 조작은 숫자 표시만 갱신** — 드래그 중 재렌더는
   슬라이더를 손에서 뺏는다
   > 2026-08-09 개정으로 창구에서 슬라이더가 사라졌다(대사 선택으로 교체). 이 규약
   > 자체는 유효하다 — **연속 입력 위젯에만 부분 갱신을 쓴다**는 뜻이고, 지금 창구는
   > 연속 입력이 없어 전체 재렌더만 쓴다. 슬라이더를 다시 도입하는 화면이 생기면
   > 이 항목이 그대로 적용된다.
...
1. **`GameState.offersMade`.** 제안 횟수를 화면이 아니라 세션 상태에 둔다. 화면에 두면
   창구를 나갔다 들어오는 것으로 **결렬 규칙을 우회**할 수 있고, `maxOffers`가
   흥정을 긴장시키는 전부다. (2026-08-09 개정으로 기본값 2 → 3.)
